...
### 🔹 倒扣挑戰：幾次後變成負數？
題目說明：從 100 開始倒扣，每次減去一個遞增的整數，直到數值變為負數，請問扣了幾次？
...
### 🔹 額外挑戰：加總過程中限制條件
題目說明：使用 while 迴圈計算 1~100 的總和，但若總和為奇數時不加，請問最後加到第幾個數？
...
while i_number < 101:
    if i_total % 2 == 0:
        ...
        i_number += 1
print(f"總和為 {i_total}，加總到第 {i_count} 個數字")
```